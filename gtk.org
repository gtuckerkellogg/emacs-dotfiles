...
#+begin_src emacs-lisp :tangle yes
  ...
  (set-face-attribute 'variable-pitch nil
    ...
    (set-face-attribute 'default nil :font "Open Sans" :weight 'light)
...
#+begin_src emacs-lisp :tangle yes
  ...
  ;      (add-to-list 'ispell-hunspell-dict-paths-alist '("en_med_glut" "/usr/share/hunspell/en_med_glut.aff"))
...
;;    (setq ispell-local-dictionary-alist '(("en_GB" "[[:alpha:]]" "[^[:alpha:]]" "[']" nil ("-d" "en_GB,en_med_glut") nil utf-8)))
      (setq ispell-personal-dictionary (expand-file-name "~/.hunspell_personal" (getenv "HOME")))
...
#+begin_src emacs-lisp :tangle yes
    (setq org-beamer-environments-extra
  	  ...
  	  ("fill" "D" "\\molochset{block=fill}\\begin{block}%a{%h}" "\\end{block}")
  	    )
    	)

    (use-package snakemake-mode
        :vc (:url "https://github.com/kyleam/snakemake-mode"
      		))
